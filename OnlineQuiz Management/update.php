...
if(@$_GET['demail'] && $_SESSION['key']=='examiner123') {
$demail=@$_GET['demail'];
...
//remove quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'rmquiz' && $_SESSION['key']=='examiner123') {
$eid=@$_GET['eid'];
...
//add quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='examiner123') {
$name = $_POST['name'];
...
//add question
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addqns' && $_SESSION['key']=='examiner123') {
$n=@$_GET['n'];
$eid=@$_GET['eid'];
$ch=@$_GET['ch'];
...
$r++;
$s=$s+$correct;
$q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`attempt`=$sn,`correct`=$r, date= NOW()  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
...
$w++;
$s=$s-$wrong;
$q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`attempt`=$sn,`wrong`=$w, date=NOW() WHERE  email = '$email' AND eid = '$eid'")or die('Error147');
}
if($sn != $total)
{
$sn++;
...
else if( $_SESSION['key']!='examiner123')
{
